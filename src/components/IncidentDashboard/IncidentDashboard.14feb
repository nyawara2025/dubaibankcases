      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto">
        {activeTab === 'incidents' ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-center animate-in fade-in duration-700">
             <div className="text-6xl mb-4 opacity-20 italic font-black uppercase">Offline</div>
             <p className="text-slate-600 font-bold uppercase tracking-[0.4em] text-[10px]">
               The {activeTab} module is currently under maintenance
             </p>
          </div>
        )}
      </main>
...
function StatCard({ icon, label, count, className = "" }) {
  return (
    <div className={`bg-slate-900/40 border border-slate-800/60 p-6 rounded-3xl backdrop-blur-sm ${className}`}>
...
function IncidentRow({ incident }) {
  return (
    <div className="px-6 py-4 flex items-center justify-between hover:bg-slate-800/20 transition-colors">
      <div>
        <div className="text-sm font-bold text-white uppercase">{incident.title || 'Untitled Breach'}</div>
        <div className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">{incident.severity} • {incident.status}</div>
      </div>
      <div className="text-xs font-mono text-indigo-400">#{incident.id.slice(0, 8)}</div>
    </div>
  );
}
